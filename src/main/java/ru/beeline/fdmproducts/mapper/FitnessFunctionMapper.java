...
import ru.beeline.fdmproducts.dto.ffunction.FitnessFunctionResponseDTO;
...
public class FitnessFunctionMapper {
    ...
    public List<GetFitnessFunctionDTO> mapFitnessFunctionDTO(List<LocalFitnessFunction> localFitnessFunctions) {
        List<GetFitnessFunctionDTO> result = new ArrayList<>();
        if (localFitnessFunctions.isEmpty()) {
            return result;
        }
        for (LocalFitnessFunction localFf : localFitnessFunctions) {
            result.add(GetFitnessFunctionDTO.builder()
                    .id(localFf.getId())
                    .code(localFf.getCode())
                    .description(localFf.getDescription())
                    .docLink(localFf.getDocLink())
                    .status(localFf.getStatus())
                    .build());
        }
        return result;
    }
