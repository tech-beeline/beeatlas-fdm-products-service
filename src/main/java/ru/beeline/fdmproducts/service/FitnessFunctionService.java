...
public class FitnessFunctionService {
    ...
    public MainResponseDTO getFitnessFunctionsAggregation() {
        ...
        List<LatestAssessmentCheckDTO> allChecks =
                localAssessmentRepository.findLatestChecksForProducts(productIds);
        ...
        Map<Integer, LacCountsDTO> countsByLacId = allLacIds.isEmpty()
                ? Collections.emptyMap()
                : localAcObjectRepository.countsByLacIds(allLacIds).stream()
                .collect(Collectors.toMap(LacCountsDTO::getLacId, dto -> dto));
        ...
        List<DomainDTO> domains = products.stream()
                ...
                .values().stream()
                .map(domain -> {
                    List<Product> domainProducts = productsByDomainId
                            .getOrDefault(domain.getId(), Collections.emptyList());

                    List<FitnessFunctionProductDTO> productDTOs = domainProducts.stream()
                            .map(product -> {
                                List<LatestAssessmentCheckDTO> checks = checksByProductId
                                        .getOrDefault(product.getId(), Collections.emptyList());

                                List<FitnessFunctionShortDTO> ffDTOs = checks.stream()
                                        .map(check -> {
                                            LacCountsDTO counts = countsByLacId
                                                    .getOrDefault(check.getLacId(), LacCountsDTO.empty());
        ...
        return MainResponseDTO.builder()
                .fitnessFunctionEnum(fitnessFunctions.stream()
